...
#define BUZZER_MUSIC_PIN 27
#define BUZZER_SOUND_PIN 17
#define BUZZER_DUMMY_PIN 21
...
int notesChannel = sizeof(melodyChannel) / sizeof(melodyChannel[0]) / 2;

int melodySound01[] = {
  NOTE_B1,8,
};
int notesSound01 = sizeof(melodySound01) / sizeof(melodySound01[0]) / 2;
...
void playTetris() {
  ...
  for (int thisNote = 0; thisNote < notesTetris * 2; thisNote = thisNote + 2) {
    ...
    tone(BUZZER_MUSIC_PIN, melodyTetris[thisNote], noteDuration*0.9);
    delay(noteDuration);    
...
void playChannel() {
  playMusic = true;
  for (int thisNote = 0; thisNote < notesChannel * 2; thisNote = thisNote + 2) {
    divider = melodyChannel[thisNote + 1];
    if (divider > 0) {
      noteDuration = (wholenote) / divider;
    } else if (divider < 0) {
      noteDuration = (wholenote) / abs(divider);
    ...
    tone(BUZZER_MUSIC_PIN, melodyChannel[thisNote], noteDuration*0.9);
    delay(noteDuration);    
    noTone(BUZZER_MUSIC_PIN);
    if(!playMusic)
      return;
...
void playSound(int id){  
  if(id==0){
    for (int thisNote = 0; thisNote < notesSound01 * 2; thisNote = thisNote + 2) {
      divider = melodySound01[thisNote + 1];
      if (divider > 0) {
        noteDuration = (wholenote) / divider;
      } else if (divider < 0) {
        noteDuration = (wholenote) / abs(divider);
      ...
      tone(BUZZER_MUSIC_PIN, melodySound01[thisNote], noteDuration*0.9);
      delay(noteDuration);    
      noTone(BUZZER_MUSIC_PIN);
